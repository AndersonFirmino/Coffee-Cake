...
#
#
# ** Coffee Cake ** is a Template for a common Cakefile that you may use in a coffeescript nodejs project.
#
...
# ## *docs*
#
# Merges every file in appFiles into one file.
# the file extension (.coffee) is not required!
#
# <small>Usage</small>
#
# ```
...
task 'build-app', 'Build single application file from source files', ->
...
# ## *docs*
#
# Minify with uglify the app.js and create one directory with name if it does not exist
#
# <small>Usage</small>#
# ```
# cake minify
...
task 'minify', 'Minifica o arquivo JS', ->
  try
    ...
    exec 'uglifyjs lib/app.js -o dist/app.min.js --source-map dist/app.map.js', (error, stdout, stderr) ->
      if error
        log error, red

    log "Arquivos minificados com sucesso", green
  catch
    console.log 'Falha ao tentar minificar, verifique o nome do arquivo.'
